...
function mergeAdminDefaults(admins: AppState["admins"]) {
  ...
  const enrichedAdmins = admins.map((admin) => {
    ...
    return {
      ...
      adminCode: admin.adminCode ?? fallback?.adminCode ?? admin.code ?? fallback?.code,
      invitationCode: admin.invitationCode ?? fallback?.invitationCode ?? admin.code ?? fallback?.code,
